...
function createData(name, cargo, correo, btn) {
  return { name, cargo, correo, btn  };
}
...
export default function TableEvaluators({evaluadores}) {

  const history = useHistory();


  const rows = evaluadores.map((items, index)=>{
    return createData(items.nombre, items.cargo, items.Email, <div>
  ...
  function eliminar(email, event){
    ...
    axios({
      method: 'post',
      url: conectividad.endPointBack+'api/evaluador/eliminar',
      data: { "Email" : email}
    }).then(function (response) {
      console.log(response);
      Swal.fire({
        title: 'Evaluador eliminado con exito',
  ...
  function actualizar(email, event){
    ...
    axios({
        method: 'post',
        url: conectividad.endPointBack+'api/evaluador/buscaruno',
        data: {"Email": email}
      }).then(function (response) {
          console.log(response.data.doc);
          data = response.data.doc
          history.push({
            pathname: '/Proveedor/Evaluador',
            datos: { 
               name : data.nombre ,
               position : data.cargo ,
               phone : data.telefono ,
